...
def check(screen,light,mirrors,shoot,sound):
    newarr=[]
    for i in mirrors:
    ...
    if(shoot):
        pygame.draw.line(screen,(252, 252, 3),light.point,(p1,p2),4)
        pygame.time.wait(20)
        sound.play()
        pygame.display.update() 
    else:
        pygame.draw.line(screen,color,light.point,(p1,p2),4)
    # print(reflector.typeof)
    if(reflector.typeof=="wall"):
        return (p1,p2),False
    if((reflector.typeof=="sprite")):
        return (p1,p2),True
    if(len(newarr)>0):
        inc_ang=angle_incident(light,reflector)
        next_ang=(-1)*(inc_ang+(180-reflector.lineangle))
        next_line=angleline(screen,(p1,p2),next_ang,(255, 233, 36),"light")
        ...
        if((light.angle>0) and (light.angle-180<0)):
           if not( (fixang(light.angle)>=reflector.lineangle) or (reflector.lineangle>=fixang(light.angle-180))):
                #next_line.show()
                return check(screen,next_line,mirrors,shoot,sound)
        else:
